...
let paidIndex=null,paidIndexSource=null;
function paidByMonth(){const payments=state.data.studentPayments;if(paidIndex&&paidIndexSource===payments)return paidIndex;const map=new Map();for(const p of payments){const key=`${Number(p.studentId)}|${p.month}`;map.set(key,(map.get(key)||0)+Number(p.amount||0))}paidIndexSource=payments;paidIndex=map;return map}
function paidFor(sid,m){return paidByMonth().get(`${Number(sid)}|${m}`)||0}
...
function statusFor(student,month){const due=feeDueFor(student,month),paid=paidFor(student.id,month),delta=due-paid,late=lateFor(student,month);if(due<=0)return paid>0?['زاد على دفعته','status-overpaid']:['لا توجد رسوم','status-exempt'];if(delta===0)return['تم الدفع','status-paid'];if(delta<0)return['زاد على دفعته','status-overpaid'];if(late||paid===0)return[paid>0?'هناك باقي':'لم يدفع بعد','status-unpaid'];return['هناك باقي','status-partial']}
