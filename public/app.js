...
async function load(){state.data=await api('/data');state.departments=await api('/departments');state.examData=await api('/exams');populateDepartments();populateExamDepartments()}
...
function resetStudent(){if(!$('studentForm'))return;$('studentForm').reset();$('studentId').value='';$('registrationDate').value=today();$('monthlyFee').value='';$('initialPaid').value='';$('callNo').value='';$('gender').value=''}
$('cancelStudent').onclick=resetStudent;
$('className').addEventListener('change',()=>{$('callNo').value=$('className').value?firstCallNo($('className').value,$('studentId').value||null):'';$('monthlyFee').value=$('className').value?departmentFee($('className').value):''})
$('studentForm').addEventListener('submit',async e=>{e.preventDefault();const p={schoolNo:western($('schoolNo').value),name:$('studentName').value,callNo:$('callNo').value,gender:$('gender').value,nni:western($('nni').value),birthPlace:$('birthPlace').value,birthDate:$('birthDate').value,guardianName:$('guardianName').value,guardianPhone:western($('guardianPhone').value),className:$('className').value,registrationDate:$('registrationDate').value,registrationFee:western($('registrationFee').value),monthlyFee:western($('monthlyFee').value),initialPaid:western($('initialPaid').value),initialPaymentMonth:currentMonth(),initialPaymentDate:today(),notes:$('studentNotes').value};try{const id=$('studentId').value;let created=null;if(id){if(!(await requirePassword()))return;created=await api(`/students/${id}`,{method:'PUT',body:JSON.stringify(p)})}else created=await api('/students',{method:'POST',body:JSON.stringify(p)});await load();resetStudent();renderStudents();renderFees();toast('تم حفظ الطالب.');if(!id&&Number(p.initialPaid)>0){const payment=state.data.studentPayments.filter(x=>Number(x.studentId)===Number(created.id)).sort((a,b)=>b.id-a.id)[0];if(payment)printStudentReceipt(payment.id)}}catch(err){toast(err.message)}});
...
function paidMonths(s){const due=Number(s.monthlyFee||0);return months.filter(m=>paidFor(s.id,m)>0).map(m=>{const paid=paidFor(s.id,m);return paid>=due?m:`${m} (جزئي)`})}
function renderStudents(){const q=$('studentSearch').value.toLowerCase().trim();const list=state.data.students.filter(s=>[s.className,s.name,s.schoolNo,s.nni].join(' ').toLowerCase().includes(q));$('studentCount').textContent=`عدد الطلاب: ${list.length}`;$('studentsTable').innerHTML=list.map(s=>{const m=currentMonth(),due=Number(s.monthlyFee||0),paid=paidFor(s.id,m),delta=due-paid,outstanding=Math.max(0,delta),over=Math.max(0,-delta),[status,cls]=statusFor(s,m),late=lateFor(s,m);return `<tr class="${late&&outstanding>0?'overdue-row':''}"><td>${esc(s.className)}</td><td>${esc(s.callNo)}</td><td>${esc(s.schoolNo)}</td><td>${esc(s.name)}</td><td>${esc(s.gender||'')}</td><td>${esc(s.nni)}</td><td>${money(due)}</td><td>${money(paid)}</td><td class="${over>0?'status-overpaid':outstanding>0?(late?'overdue-strong':'overdue-soft'):'status-paid'}">${over>0?money(over)+' زيادة':money(outstanding)}</td><td class="${cls}">${status}</td><td class="paid-months">${esc(paidMonths(s).join('، ')||'لا يوجد')}</td><td><div class="student-payment-box"><input id="sp-${s.id}" class="payment-input" type="number" min="1"><button class="btn-pay" onclick="payCurrentStudent(${s.id})">حفظ</button></div></td><td class="actions"><button class="btn-edit" onclick="editStudent(${s.id})">تعديل</button><button class="btn-delete" onclick="removeStudent(${s.id})">حذف</button></td></tr>`}).join('')}
window.editStudent=id=>{const s=state.data.students.find(x=>x.id===id);if(!s)return;for(const [id2,v] of Object.entries({studentId:s.id,className:s.className,callNo:s.callNo,schoolNo:s.schoolNo,studentName:s.name,gender:s.gender||'',nni:s.nni,birthPlace:s.birthPlace,birthDate:s.birthDate,guardianName:s.guardianName,guardianPhone:s.guardianPhone,registrationDate:s.registrationDate,registrationFee:s.registrationFee,monthlyFee:s.monthlyFee,studentNotes:s.notes}))$(id2).value=v??'';go('students')}
...
function loadExamEntry(){
  ...
  $('examEntryArea').innerHTML=`<div class="table-scroll"><table class="exam-entry-table"><thead><tr><th>المادة</th>${primary?'<th>النتيجة</th>':'<th>الاختبارات</th><th>الامتحان</th><th>الضارب</th><th>المجموع</th>'}</tr></thead><tbody>`+
...
function printExamRecord(id){
  ...
  const sheet=`<div id="printSheet" class="results-sheet"><div class="results-header"><div>${esc(h.republic||'الجمهورية الإسلامية الموريتانية')}</div><div>${esc(h.ministry||'وزارة التعليم')}</div><div>${esc(h.regional||'الإدارة الجهوية للتعليم')}</div><div class="school-title">${esc(state.settings.schoolName||'')}</div><div class="phone">الهاتف: ${esc(h.schoolPhone||state.settings.schoolPhone||'')}</div><div>${esc(state.settings.schoolYear||'')}</div><div class="exam-title">كشف الامتحان ${esc(r.examNo)}</div></div><div class="results-student-meta">${meta}</div><table><thead><tr>${cols}</tr></thead><tbody>${rows}</tbody></table><div class="results-footer"><div>المجموع<br>${money(c.sum)}</div><div>مجموع الضوارب<br>${primary?'—':money(c.coeffSum)}</div><div>المعدل<br>${money(c.avg)}</div><div>الملاحظة<br>${esc(c.remark)}</div><div>القرار<br>${esc(c.decision)}</div></div></div>`;
